...
Fixes applied vs. the earlier notebook versions:
  - Uses the REAL column name "Fields_of_Study" (plural) everywhere --
    earlier versions mixed "Field_of_Study" (singular) and
    "Fields_of_Study" (plural), which crashed with a KeyError depending on
    which cell last ran.
  - Ranking filters/sorts now use "QS_Rank_Numeric" (a clean int column,
  ...
  - Scholarship matching now checks for the exact "Yes" category instead
  ...
  - `df` and `X` (embeddings matrix) are explicit parameters, not globals
    ...
    class of bugs seen in the earlier versions.
  - Country / field-of-study text matching now uses regex=False, so a
    user typing something like "C++ " or "(Design)" can't blow up
